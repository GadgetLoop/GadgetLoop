...
// const button2 = (req, res) => {
//     res.send('<a href="/Update">Authenticate with Google</a>');
// };
...
const updatepassword = async (req, res) => {
    ...
    try {
        ...
        if (validate.error) {
            ...
            const emailCheck = await db.query(checkEmailQuery, [email]);
            if (emailCheck.rows.length > 0) {
                // Send the verification email if it hasn't been sent yet
                ...
                if (verificationCode === generatedVerificationCode) {
                    ...
                    res.status(200).json({
                        message: 'Verification code is correct. You can now proceed with the password update.',
                    });
                } else {
                    res.status(400).json({
